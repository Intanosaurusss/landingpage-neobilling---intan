...
<header class="flex flex-wrap fixed md:justify-start md:flex-nowrap z-50 w-full py-4 bg-orange-400">
  <nav class="relative max-w-7xl w-full flex flex-wrap md:grid md:grid-cols-12 basis-full items-center px-4 md:px-6 md:px-8 mx-auto">
    <div class="md:col-span-3 flex items-center gap-2">
      <!-- Logo -->
       <div class="">
        <img src="{{ asset('assets/logoneo.png') }}" alt="image description" class="rounded-full bg-white w-12 h-12 p-2">
       </div>
      <!-- End Logo -->

      <!-- Teks hanya muncul di layar md ke atas -->
      <span class="hidden md:block text-lg font-semibold text-white">Neobilling</span>
    </div>

    <!-- Button Group -->
    <div class="flex items-center gap-x-1 md:gap-x-2 ms-auto py-1 md:order-3 md:col-span-3">
    <!-- Tombol Account -->
    <a href="https://my.idneobilling.com/auth/register" type="button" class="py-2 px-4 md:border-2 border border-orange-300 md:border-orange-600 inline-flex items-center gap-x-2 text-sm font-medium rounded-xl md:rounded-full hover:bg-orange-600 focus:outline-none disabled:opacity-50 disabled:pointer-events-none dark:text-white dark:focus:text-white">
        <!-- Teks Account hanya tampil di layar medium ke atas -->
        <span class="hidden md:block text-white">Akun saya</span>
        <!-- Ikon hanya tampil di layar kecil -->
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="text-white block md:hidden w-5 h-5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M17.982 18.725A7.488 7.488 0 0 0 12 15.75a7.488 7.488 0 0 0-5.982 2.975m11.963 0a9 9 0 1 0-11.963 0m11.963 0A8.966 8.966 0 0 1 12 21a8.966 8.966 0 0 1-5.982-2.275M15 9.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
        </svg>
    </a>

    <!-- Tombol Chat On WhatsApp -->
    <a href="https://wa.me/6281384329600" target="_blank" rel="noopener noreferrer">
    <button type="button" class="py-2 px-3 inline-flex items-center gap-x-2 text-sm font-medium rounded-xl border border-orange-300 md:border-2 md:border-transparent sm:bg-transparent md:bg-orange-600 text-white hover:bg-orange-400 hover:border-orange-600 focus:outline-none disabled:opacity-50 disabled:pointer-events-none">
        <!-- Teks Chat On WhatsApp hanya tampil di layar medium ke atas -->
        <span class="hidden md:block">Hubungi di Whatsapp</span>
        <!-- Ikon hanya tampil di layar kecil -->
    ...
    <div id="hs-navbar-hcail" class="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow md:block md:w-auto md:basis-auto md:order-2 md:col-span-6 bg-navbar-default md:bg-transparent rounded-lg pl-4 pb-4 pt-2 z-50 mt-2" aria-labelledby="hs-navbar-hcail-collapse">
      <div class="flex flex-col gap-y-4 gap-x-0 mt-5 md:flex-row md:justify-center md:items-center md:gap-y-0 md:gap-x-7 md:mt-0">
        <div>
          <a href="#home" class="relative inline-block text-white hover:text-orange-600 focus:outline-none dark:text-white" href="#" aria-current="page" id="link-home">Beranda</a>
        </div>
        <div>
          <a href="#about" class="inline-block text-white hover:text-orange-600 focus:outline-none dark:text-white dark:hover:text-orange-600" id="link-about">Tentang</a>
        </div>
        <div>
          <a href="#services" class="inline-block text-white hover:text-orange-600 focus:outline-none dark:text-white dark:hover:text-orange-600"id="link-services">Layanan</a>
        </div>
        <div>
          <a href="#price" class="inline-block text-white hover:text-orange-600 focus:outline-none dark:text-white dark:hover:text-orange-600" id="link-price">Harga</a>
        </div>
        <!-- <div>
          <a href="#faq" class="inline-block text-white hover:text-orange-600 focus:outline-none dark:text-white dark:hover:text-orange-600" id="link-faq">FAQ</a>
        </div> -->
        <div>
          <a href="#contact" class="inline-block text-white hover:text-orange-600 focus:outline-none dark:text-white dark:hover:text-orange-600" id="link-contact">Kontak</a>
        </div>
      </div>
    </div>
    <!-- End Collapse -->
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const sections = ['home', 'about', 'services', 'price', 'contact'];
